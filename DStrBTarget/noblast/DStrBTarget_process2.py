...
#output:target, reference ligand
import sys
...
def append_score(targ, lig2, score):
...
def add_12(sett,lig_simi,lig2i):
    for targi in sett:
        ...
        append_score(targi, lig2i, score12)#Integration of 2D+3D+bind score
...
def score12(lig1i):#Generating 2D+3D+bind score for the input ligand to each ref-target. Note that lig1i means index of lig1 in molSet.
    ...
    for targi in scoreSet.keys():#Convient of later integration of blast
        scoreSet[targi].sort(key=lambda x:x[1],reverse=True)
        scoreSet[targi]=scoreSet[targi][0] #highest 2D+3D+bind score
    return scoreSet
...
#read_data#
    ...
    refbindSet=read_bindaff('../database/fact190605_afteradd3_scaled.lnBA1w26',ligSet)
    ...
    for lig1i in [ligIn.index(lig1) for lig1 in ligIn]:
        ...
        sortedSet=sorted(scoreSet.items(),key=lambda x:float(x[1][1]),reverse=True)
        sortedSet=sortedSet[:14] #top15 result
        resultfile=open(ligIn[lig1i]+'.result','a+')
        resultfile.write('targ_uniprot '+'score '+'reference_lig '+'2Dsimi '+'3Dsimi '+'normalized_bindaff'+'\n')
        for x in sortedSet:
            targ=targSet[x[0]]
            score=x[1][1]
            lig2=ligSet[x[1][0]]
            bindvalue=float(refbindSet[x[1][0]][x[0]])
            D2simi=float(taniSet[9197*lig1i+x[1][0]])
            D3simi=float(LSSet[9197*lig1i+x[1][0]])
            resultfile.write('{} {:3f} {} {:3f} {:3f} {:3f}\n'.format(targ, score, lig2, D2simi, D3simi, bindvalue))
        resultfile.close()
